    ...
    if [ -f "$USERS_FILE" ]; then
        if systemctl is-active --quiet dijiq-server.service; then
            ...
            if jq -e "has(\"$username\")" "$USERS_FILE" > /dev/null; then
                ...
                if [ "$generate_singbox" = true ] && systemctl is-active --quiet dijiq-singbox.service; then
                    read -r domain port < <(get_singbox_domain_and_port)
                    if [ -n "$domain" ] && [ -n "$port" ]; then
                        echo -e "\nSingbox Sublink:\nhttps://$domain:$port/sub/singbox/$username/$ip_version#$username\n"
                    fi
                fi
                if [ "$generate_normalsub" = true ] && systemctl is-active --quiet dijiq-normal-sub.service; then
                    read -r domain port subpath < <(get_normalsub_domain_and_port)
                    if [ -n "$domain" ] && [ -n "$port" ]; then
                        echo -e "\nNormal-SUB Sublink:\nhttps://$domain:$port/$subpath/sub/normal/$username#dijiq\n"
                    fi
                fi
            else
            ...
            echo -e "\033[0;31mError:\033[0m dijiq is not active."
        fi
    else
